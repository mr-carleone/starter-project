...
async def create_role(role_data: RoleCreate, db: AsyncSession = Depends(get_db)):
    service = RoleService(db)
    return await service.create_role(role_data.name)
...
async def update_role(
    role_id: UUID, role_data: RoleUpdate, db: AsyncSession = Depends(get_db)
):
    service = RoleService(db)
    return await service.update_role(role_id, role_data.name)
